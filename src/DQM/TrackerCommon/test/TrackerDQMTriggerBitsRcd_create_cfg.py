...
process.TrackerDQMTriggerBitsRcdCreateNew = cms.EDAnalyzer( "AlCaRecoTriggerBitsRcdUpdate"
                                                          , firstRunIOV = cms.uint32( 1 )
                                                          , lastRunIOV  = cms.int32( -1 )
                                                          , startEmpty = cms.bool( True )
                                                          , listNamesRemove = cms.vstring()
                                                          , triggerListsAdd = cms.VPSet( cms.PSet( listName = cms.string( 'TrackerDQM_Gt' )
                                                                                                 , hltPaths = cms.vstring( 'PhysicsDeclared'
                                                                                                                         )
                                                                                                 )
                                                                                       , cms.PSet( listName = cms.string( 'TrackerDQM_L1_Tracking' )
...
process.PoolDBOutputService = cms.Service( "PoolDBOutputService"
                                         ...
                                         , connect  = cms.string( 'sqlite_file:/afs/cern.ch/user/v/vadler/cms/SiStripDQM/CMSSW_3_5_4/output/TrackerDQMTriggerBits.db' )
...
process.p = cms.Path( process.TrackerDQMTriggerBitsRcdCreateNew
                    )
