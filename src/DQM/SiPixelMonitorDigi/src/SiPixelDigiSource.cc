...
SiPixelDigiSource::SiPixelDigiSource(const edm::ParameterSet& iConfig) :
  ...
  triggerFlag_( new GenericTriggerEventFlag( iConfig ) )
{
...
SiPixelDigiSource::~SiPixelDigiSource()
{
  ...
  if ( triggerFlag_ ) delete triggerFlag_;
}
...
void SiPixelDigiSource::beginRun(const edm::Run& r, const edm::EventSetup& iSetup){
  ...
  if ( triggerFlag_->on() ) triggerFlag_->initRun( r, iSetup );
}
...
//  if(nLumiSecs%5==0){
  ...
  bool eventFlag( triggerFlag_->accept( iEvent, iSetup ) );

  // Rate of events with >N digis:
  if(nEventDigis>bigEventSize){
    me = theDMBE->get("Pixel/bigEventRate");
    if(me && eventFlag) me->Fill(lumiSection,1./23.);
  }
  ...
  if(nActiveModules>=4){
    ...
    if(me && eventFlag) me->Fill(float(bx));
    me1 = theDMBE->get("Pixel/pixEventRate");
    if(me1 && eventFlag) me1->Fill(lumiSection, 1./23.);
  }
