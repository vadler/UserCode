...
void AnalyzeHitFitResolutionFunctions::beginJob()
{
  ...
  for ( unsigned iCat = 0; iCat < objCats_.size(); ++iCat ) {
    ...
    for ( unsigned iProp = 0; iProp < kinProps_.size(); ++iProp ) {
      ...
      for ( unsigned iEta = 0; iEta < etaBins_.at( iCat ).size() - 1; ++iEta ) {
        const std::string eta( my::helpers::to_string( iEta ) );
        ...
        const std::string nameDirRecoEta( nameDirReco + "_" + eta );
        TFileDirectory subDirRecoEta( subDirReco.mkdir( nameDirRecoEta.c_str(), title.c_str() ) );
        ...
        const std::string nameDirGenEta( nameDirGen + "_" + eta );
        TFileDirectory subDirGenEta( subDirGen.mkdir( nameDirGenEta.c_str(), title.c_str() ) );
        ...
        const std::string nameDirRecoInvEta( nameDirRecoInv + "_" + eta );
        TFileDirectory subDirRecoInvEta( subDirRecoInv.mkdir( nameDirRecoInvEta.c_str(), title.c_str() ) );
        ...
        const std::string nameDirGenInvEta( nameDirGenInv + "_" + eta );
        TFileDirectory subDirGenInvEta( subDirGenInv.mkdir( nameDirGenInvEta.c_str(), title.c_str() ) );
      ...
      for ( unsigned iEtaSymm = 0; iEtaSymm < etaSymmBins_.at( iCat ).size() - 1; ++iEtaSymm ) {
        const std::string etaSymm( my::helpers::to_string( iEtaSymm ) );
        ...
        const std::string nameDirRecoSymmEta( nameDirRecoSymm + "_" + etaSymm );
        TFileDirectory subDirRecoSymmEta( subDirRecoSymm.mkdir( nameDirRecoSymmEta.c_str(), title.c_str() ) );
        ...
        const std::string nameDirGenSymmEta( nameDirGenSymm + "_" + etaSymm );
        TFileDirectory subDirGenSymmEta( subDirGenSymm.mkdir( nameDirGenSymmEta.c_str(), title.c_str() ) );
        ...
        const std::string nameDirRecoInvSymmEta( nameDirRecoInvSymm + "_" + etaSymm );
        TFileDirectory subDirRecoInvSymmEta( subDirRecoInvSymm.mkdir( nameDirRecoInvSymmEta.c_str(), title.c_str() ) );
        ...
        const std::string nameDirGenInvSymmEta( nameDirGenInvSymm + "_" + etaSymm );
        TFileDirectory subDirGenInvSymmEta( subDirGenInvSymm.mkdir( nameDirGenInvSymmEta.c_str(), title.c_str() ) );
