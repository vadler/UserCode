...
class TopHitFitResolutionFunctions : public edm::EDAnalyzer {
  ...
  private:
    ...
    void fill1D( unsigned iCat, MapTH1D & histMap1D, unsigned iBin = 0 );
    void fill2D( unsigned iCat, MapTH2D & histMap2D, unsigned iBin = 0 );
    ...
    if ( ttGenEvent_->isTtBar() && ttGenEvent_->isSemiLeptonic( WDecay::kMuon ) ) { // semi-muonic signal event
      ...
      if ( ttSemiLeptonicEvent_->isHypoValid( TtEvent::kGenMatch ) ) {
        for ( unsigned iCat = 0; iCat < objCat_.size(); ++iCat ) {
          ...
          fill1D( iCat, histMap1D_, nBins );
          fill2D( iCat, histMap2D_, nBins );
...
void TopHitFitResolutionFunctions::fill1D( unsigned iCat, MapTH1D & histMap1D, unsigned iBin )
{
  ...
  switch ( iCat ) {
    case 0:
      ...
      break;
    case 1:
      eta = ttSemiLeptonicEvent_->hadronicDecayQuark( TtEvent::kGenMatch )->eta();
      if ( etaMin <= eta && eta < etaMax ) {
      ...
      eta = ttSemiLeptonicEvent_->hadronicDecayQuarkBar( TtEvent::kGenMatch )->eta();
      if ( etaMin <= eta && eta < etaMax ) {
      ...
      break;
    case 2:
      eta = ttSemiLeptonicEvent_->hadronicDecayB( TtEvent::kGenMatch )->eta();
      if ( etaMin <= eta && eta < etaMax ) {
      ...
      eta = ttSemiLeptonicEvent_->leptonicDecayB( TtEvent::kGenMatch )->eta();
      if ( etaMin <= eta && eta < etaMax ) {
...
void TopHitFitResolutionFunctions::fill2D( unsigned iCat, MapTH2D & histMap2D, unsigned iBin )
{
